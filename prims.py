...
def prims_algorithm(graph, start_vertex):
    ...
    while not heap.is_empty():
        ...
        if current_vertex.key in vertex_optimum_path:
            result.append(vertex_optimum_path[current_vertex.key])

        for next_vertex in graph.neighbours(current_vertex.key):
            if next_vertex in heap:
                new_cost = graph.get_weight(current_vertex.key, next_vertex)
                current_cost = heap[next_vertex].value
                if new_cost < current_cost:
                    vertex_optimum_path[next_vertex] = (current_vertex.key, next_vertex)
...
if __name__ == "__main__":
    ...
    result = prims_algorithm(graph, start_vertex)

    total_weight = 0
    for i in result:
        u = i[0]
        v = i[1]
        total_weight = total_weight + graph.get_weight(u, v)
    print(total_weight)
